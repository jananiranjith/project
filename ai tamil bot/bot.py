...
from ctransformers import AutoModelForCausalLM  # For GGUF model

# ====== Setup ======
st.set_page_config(page_title="Tamil RAG Chatbot", page_icon="🗣️", layout="centered")
...
# ====== Load GGUF Language Model using ctransformers ======
@st.cache_resource
...
def speak_and_return_audio(text, lang):
...
if st.button("Send"):
    if text_input.strip():
        ...
        st.session_state.messages.append({"role": "bot", "text": response, "lang": lang})
    else:
...
for msg in st.session_state.messages:
    if msg["role"] == "user":
        ...
        st.markdown(speak_and_return_audio(msg["text"], msg["lang"]), unsafe_allow_html=True)
